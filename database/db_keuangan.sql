...
-- Set the delimiter to something else (i.e., //)
DELIMITER //
...
CREATE DEFINER=`root`@`localhost` FUNCTION `kode_automatis` (`kode` INT) RETURNS CHAR(7) CHARSET latin1 DETERMINISTIC
BEGIN
...
CREATE DEFINER=`root`@`localhost` FUNCTION `kode_automatis2` (`kode` INT) RETURNS CHAR(7) CHARSET latin1
DETERMINISTIC
BEGIN
DECLARE kodebaru CHAR(7);
DECLARE urut INT;
